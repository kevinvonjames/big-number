...
export class FloatingBoxManager {
	...
	async getTodayNumber(settings: BoxSettings): Promise<string> {
		...
		switch (settings.dataType) {
			...
			case "characterCount":
				return this.countCharacters(content).toString();
			case "sentenceCount":
				return this.countSentences(content).toString();
			case "pageCount":
				return this.getPageCount(
					content,
					settings.pageWordsPerPage
				).toString();
			case "dataview": {
	...
	private getPageCount(content: string, pageWords: number): number {
		const wordCount = this.countWords(content);
		if (!pageWords || pageWords <= 0) pageWords = 250; // fallback to default
		return parseFloat((wordCount / pageWords).toFixed(1));
	}
